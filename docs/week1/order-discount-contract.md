...
| `COND-003` | 동일한 요청은 처음 결정된 주문 할인 적용 결과를 재사용한다.                                         |
...
| ID     | 질문                                                                                                                                            | 답에 따라 달라지는 항목                                                                                          |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `Q-01` | ‘보유 쿠폰’은 구매자 본인에게 발급·귀속된 쿠폰만 의미하는가, 다른 구매자에게 공유받은 쿠폰도 포함하는가?                                                                                  | **사용자 응답:** 공유받은 쿠폰의 주문 할인 적용 성공 또는 권한 부족 오류 중 반환할 결과가 달라진다.<br>**권한:** 보유 쿠폰으로 인정할 구매자와 쿠폰의 관계가 달라진다. |
| `Q-02` | 할인 금액이 원금을 초과하면 할인 금액을 원금으로 제한할 것인가, 주문 할인 적용을 거절할 것인가?                                                                                       | **사용자 응답:** 0원으로 제한된 주문 할인 적용 결과 또는 주문 할인 적용 불가 오류가 반환된다.                                              |
| `Q-03` | 주문 확정 전, 이미 쿠폰이 적용된 주문에 다른 쿠폰으로 주문 할인 적용을 요청하면 어떤 흐름으로 처리할 것인가?<br>예: 기존 쿠폰을 해제하고 새 쿠폰으로 교체하거나, 요청을 거절하고 구매자가 기존 쿠폰을 명시적으로 해제한 뒤 다시 요청하도록 한다. | **사용자 응답:** 적용된 쿠폰이 새 쿠폰으로 변경된 주문 할인 적용 결과 또는 기존 쿠폰의 해제가 필요하다는 오류 중 반환할 결과가 달라진다.                      |
| `Q-04` | 시스템 접수 시각과 쿠폰 만료 시각이 같으면 쿠폰을 유효한 것으로 판단할 것인가, 만료된 것으로 판단할 것인가?                                                                                | **사용자 응답:** 주문 할인 적용 성공 또는 쿠폰 만료 오류 중 반환할 결과가 달라진다.                                                    |
...
이 문서에서 사용하는 제품 용어와 현재까지 확인된 의미를 정리한다. 제품 개념은 아래 표제어로만 표현하며 유사한 다른 용어를 혼용하지 않는다.


| 용어                       | 현재 확인된 의미                                                    | 확정 상태                                                  |
| ------------------------ | ------------------------------------------------------------ | ------------------------------------------------------ |
| 구매자                      | 주문 확정 전에 주문 할인 적용을 요청하는 사람이다.                                | 제품 약속으로 확정 (`FACT-001`)                                |
| 주문 할인 적용                 | 주문 확정 전의 주문에 쿠폰 한 장을 적용하여 할인 금액과 최종 금액을 결정하는 행위다.            | 제품 약속과 실습 조건으로 확정 (`FACT-001`, `COND-001`, `COND-002`) |
| 주문 확정                    | 결정된 할인 금액이 이후 쿠폰 정책이 변경되어도 바뀌지 않게 되는 기준 상태다.                 | 제품 약속으로 확정 (`FACT-002`)                                |
| 보유 쿠폰                    | 구매자가 주문에 적용할 자격이 있는 쿠폰이다.                                    | 부분 확정 · `Q-01` 확인 필요                                   |
| 원금 (`originalAmount`)    | 주문 할인 적용 전의 주문 금액이다.                                         | 실습 범위에서 확정 (`COND-002`)                                |
| 할인 금액 (`discountAmount`) | 주문 할인 적용으로 원금에서 차감하기로 결정된 금액이다.                              | 실습 범위에서 확정 (`COND-002`)                                |
| 최종 금액 (`finalAmount`)    | 원금에서 할인 금액을 차감한 금액이며 0원보다 작을 수 없다. 원금 초과 할인 처리 방식은 정해지지 않았다. | 계산식과 금액 경계 확정 · `Q-02` 확인 필요                           |
| 동일한 요청                   | 같은 구매자가 같은 주문과 같은 쿠폰에 대해 반복한 주문 할인 적용 요청이다.                  | 실습 범위에서 확정 (`COND-003`)                                |
| 적용된 쿠폰                   | 주문 할인 적용 결과를 결정하는 데 사용된 쿠폰이다.                                | 수량 확정 · `Q-03` 확인 필요                                   |
...
| `INV-004` | 최종 금액은 `finalAmount = originalAmount - discountAmount`로 계산하며 0원보다 작을 수 없다. | 실습 조건 (`COND-002`) | 원금이 10,000원이고 할인 금액이 12,000원인 주문의 최종 금액을 -2,000원으로 계산한다. | 최종 금액을 음수로 확정하지 않는다. 원금 초과 할인 처리는 `Q-02`의 답을 따르며, 제품 답변을 받으면 주문 최종 금액 계산 경계가 해당 정책을 책임진다.              | 주문 최종 금액 계산         |
| `INV-005` | 동일한 요청은 처음 결정된 주문 할인 적용 결과를 재사용한다.                                         | 실습 조건 (`COND-003`) | 동일한 요청을 두 번 보낸다.                                         | 처음 결정된 주문 할인 적용 결과를 그대로 반환하고 할인 금액을 중복으로 차감하지 않는다.                                                     | **주문 할인 적용 멱등성 보장** |
...
`INV-001..006`의 반례 중 주문 할인 적용 요청이 실패해야 하는 경우를 정의한다. `INV-002`의 반례는 후속 주문 확정·조회 계약에서 검증하고, `INV-005`의 반례는 처음 결정된 주문 할인 적용 결과를 성공 응답으로 반환하므로 포함하지 않는다. 제품 답변이 필요한 항목은 실패 여부가 정해지지 않았으므로 이 표에 포함하지 않는다.
...
| `INV-001` | 주문 확정 이후 주문 할인 적용을 요청함            | `409 Conflict`              | `ORDER_ALREADY_CONFIRMED` | 주문 확정 이후에는 주문 할인 적용을 할 수 없습니다.  |
...
| --------------------------- | ----------------- | ---------------- | ------------------------------- | ----- |
| `orderId` 또는 요청 본문의 형식이 잘못됨 | `400 Bad Request` | `Bad Request`    | 요청 형식이 올바르지 않습니다. 입력값을 확인해 주세요. | 없음    |
| 주문이 존재하지 않음                 | `404 Not Found`   | `Not Found`      | 주문을 찾을 수 없습니다. 주문 ID를 확인해 주세요.  | 없음    |
| 쿠폰이 존재하지 않음                 | `404 Not Found`   | `Not Found`      | 쿠폰을 찾을 수 없습니다. 쿠폰 ID를 확인해 주세요.  | 없음    |
...
**주문 할인 적용 멱등성 보장(`INV-005`)**은 동일한 요청이 반복되었을 때 처음 결정된 주문 할인 적용 결과를 반환하고, 할인 금액을 중복으로 차감하지 않도록 책임진다. 판단 시점은 주문 할인 적용 요청을 처리할 때다.
...
| 주문 할인 적용 결과 보존  | 주문 할인 적용 시 결정된 주문 할인 적용 결과를 스냅샷으로 보존한다.                                                                    | 주문 ID와 쿠폰 ID로 현재 원금과 쿠폰 정책을 다시 조회하여 주문 할인 적용 결과를 재계산한다. | 원금과 쿠폰 정책은 주문 할인 적용 이후 변경될 수 있으므로 현재 정보를 다시 조회해 계산하면 동일한 요청의 결과가 달라질 수 있다. 동일한 요청을 반복할 때 결과가 달라지면 `INV-005`를 위반하므로 처음 결정된 주문 할인 적용 결과를 보존한다. |
