...
- 아래 조건은 제품 담당자와 논의하고 확정한 조건이다. `COND-002`의 최종 금액이 0원보다 작을 수 없다는 금액 경계도 제품 담당자에게 확인된 조건이다.
...
이 문서에서 동일한 요청은 동일한 구매자·주문 ID·쿠폰 ID로 구성된 주문 할인 적용 요청을 뜻한다. 그리고 주문, 쿠폰  내부 내용도 변경이 없다.

### 제품 담당자 확인 필요한 질문

**현재 제품 요구사항으로부터 파생되는가**, **주문 할인 적용 유즈케이스의 성공·실패에 직접 영향을 주는가**를 기준으로 질문을 선별한다.

이유는, 현재 요구사항과 직접 맞닿지 않은 질문은 확인된 근거 없이 제품 정책을 주관적으로 파생시키고 계약의 불필요한 복잡도를 높일 수 있다고 판단하기 때문이다.
...
| --------- | -------------------------------------------------------------------------- | ------------------ | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------- |
| `INV-001` | 구매자는 주문 확정 전에 보유 쿠폰을 적용할 수 있다.                                             | 제품 약속 (`FACT-001`) | 구매자가 보유 쿠폰이 아닌 쿠폰으로 주문 할인 적용을 요청한다.                   | 요청은 거절되고 적용된 쿠폰과 주문 할인 적용 결과는 변경되지 않는다. 보유 쿠폰으로 인정할 범위는 `Q-01`의 답을 따른다.                                           | **주문 할인 적용 자격 판정**  |
