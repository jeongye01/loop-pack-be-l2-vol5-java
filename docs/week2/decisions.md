...
**상황**: 주문 응답은 품목과 합계를 항상 포함한다. `spring.jpa.open-in-view`는 `false`이고 application이 반환한 `Order`를 interfaces에서 응답으로 바꾸므로, 트랜잭션이 끝난 뒤 지연 로딩된 품목에 접근하면 `LazyInitializationException`이 발생한다. 실제 HTTP 테스트에서 주문 조회가 이 이유로 실패했다.
...
| 문제 | 대응 |
| --- | --- |
| 품목이 필요 없는 주문 조회가 생겨도 항상 품목을 읽는다. | 현재는 그런 흐름이 없다. 주문 요약 조회가 생기면 B나 C로 전환한다. |
| 주문당 품목 수가 커지면 조회 비용이 증가한다. | 현재 요구사항은 일반적인 주문 품목 범위이며, 목록 조회는 주문 페이지를 먼저 확정한다. 성능 요구가 생기면 실제 쿼리 수와 데이터 양을 측정한다. |

**다시 검토할 조건**: 품목 없는 주문 요약 조회가 생기거나, 주문당 품목 수 증가로 조회 비용이 문제가 되거나, application이 응답 전용 결과를 반환하도록 경계를 바꿀 때
