...
- 참여자는 계층이다. 요청은 interfaces → application → domain 순서로 들어가고, 저장은 application이 요청해 infrastructure가 DB에 반영한다.
- 리포지토리는 domain이 정의하고 infrastructure가 구현한다. 그림에서는 구현하는 쪽인 infrastructure에 둔다.
- 고객 요청은 `X-USER-ID`로 식별한다. 관리자 요청은 관리자 접근 필터가 `ADMIN` 역할을 확인한 뒤에만 interfaces에 닿는다.
- 상품 가격과 재고 같은 숫자는 흐름을 보이기 위해 준비한 값이다.
