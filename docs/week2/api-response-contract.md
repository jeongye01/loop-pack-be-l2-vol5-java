...
- 성공은 `200 OK`이고 `meta.result`는 `SUCCESS`이다. 돌려줄 값이 없으면 `data`는 `null`이다.
...
| `422` | `BRAND_CHANGE_NOT_ALLOWED` | 상품 수정에서 브랜드를 바꾸려 했다. | 상품의 브랜드는 바꿀 수 없습니다. |
| `422` | `PRODUCT_NOT_AVAILABLE` | 확정하려는 주문의 상품이 삭제되었다. | 주문한 상품 중 판매하지 않는 상품이 있습니다. |
| `422` | `INSUFFICIENT_STOCK` | 확정하려는 주문의 수량보다 재고가 적다. | 재고가 부족합니다. |
| `422` | `INSUFFICIENT_POINT` | 확정하려는 주문의 금액보다 잔액이 적다. | 포인트 잔액이 부족합니다. |
| `422` | `POINT_BALANCE_LIMIT_EXCEEDED` | 충전 후 잔액이 표현할 수 있는 범위를 넘는다. | 충전할 수 있는 한도를 넘었습니다. |

`409`는 저장된 상태와 부딪히는 요청, `422`는 형식은 맞지만 업무 규칙을 통과하지 못한 요청에 쓴다.
