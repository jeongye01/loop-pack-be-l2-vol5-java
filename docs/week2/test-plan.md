...
| ------------- | ----- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Stock         | 상품    | `product/domain/StockTest`       | 수량은 0 이상이다. · 차감하면 줄어든 수량이 된다(5에서 2를 빼면 3). · 재고보다 많이 차감하면 거절한다(5에서 6). · 차감할 수량이 0 이하이면 내부 오류로 거절한다(0, -1). | R-ADMIN-08, R-ORDER-08, R-ORDER-10, R-ORDER-11, ADR-003 |
...
| 테스트 파일 | 배치된 요구사항 |
| --- | --- |
| `product/domain/StockTest` | R-ADMIN-08, R-ORDER-08, R-ORDER-10, R-ORDER-11, ADR-003 |
...
| 질문 | 드러난 곳 | 판단 |
| --- | --- | --- |
| 재고 차감 수량이 0 이하이면 Stock은 어떻게 하는가? 주문 수량은 양수(R-ORDER-06)라 주문에서는 들어오지 않지만, Stock 자체의 규칙은 없다. | Stock 오류 추측 | 내부 오류(`INTERNAL_ERROR`)로 거절하고 재고는 그대로 둔다. ([ADR-003](./decisions.md#adr-003-재고-차감-수량이-0-이하이면-내부-오류로-거절한다)) |
| 이름의 앞뒤 공백과 대소문자가 다르면 같은 이름인가? 길이는 앞뒤 공백을 빼고 세는가? | 이름 경계값, 이름 중복 오류 추측 | 정하지 않았다. |
| 주문 확정에서 여러 조건이 함께 실패하면 어떤 오류를 알리는가? | 주문 확정 의사결정표 | 우선 시퀀스 다이어그램의 확인 순서(본인 → `DRAFT` → 상품 → 재고 → 잔액)를 따른다. |
